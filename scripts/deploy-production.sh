...
# InvenFlow Production Deployment Script - Port 3002
...
APP_NAME="invenflow-production"

echo -e "${BLUE}🚀 Starting InvenFlow PRODUCTION deployment (Port 3002)...${NC}"
...
"$PROJECT_ROOT/scripts/build-production.sh" || {
...
"$PROJECT_ROOT/scripts/migrate-production.sh" || {
...
pm2 start pm2/ecosystem.config.cjs --only "$APP_NAME" --env production || {
